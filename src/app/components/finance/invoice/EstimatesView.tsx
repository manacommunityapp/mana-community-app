...
export function EstimatesView({ onNewEstimate, canAdd = true }: { onNewEstimate: () => void; canAdd?: boolean }) {
  ...
  const [estimates, setEstimates] = useState<Array<{ id: string; date: string; customer: string; amount: number; status: string }>>([]);

  useEffect(() => {
    (async () => {
      try {
        const docs = await ledgerFinanceService.getDocuments("ESTIMATE");
        setEstimates(docs.map((d) => ({
          id: d.code || `#${d.id}`,
          date: d.docDate || "",
  ...
  const metrics = useMemo(() => {
    ...
    estimates.forEach((est) => {
      total += est.amount;
      if (est.status === "Under review") underReview += est.amount;
      if (est.status === "Approved") approved += est.amount;
      if (est.date.startsWith("2026-07")) thisMonth += est.amount;
    });

    return { total, underReview, approved, thisMonth };
  }, [estimates]);
  ...
  const filteredEstimates = useMemo(() => {
    return estimates.filter((est) => {
      ...
      if (searchQuery && !est.id.toLowerCase().includes(searchQuery.toLowerCase()) && !est.customer.toLowerCase().includes(searchQuery.toLowerCase())) return false;

      // Period filter
      if (periodFilter === "This Month" && !est.date.startsWith("2026-07")) return false;
      if (periodFilter === "Last Month" && !est.date.startsWith("2026-06")) return false;

      // Advanced filters
      if (customerFilter && est.customer !== customerFilter) return false;
      if (minAmount && est.amount < parseFloat(minAmount)) return false;
      if (maxAmount && est.amount > parseFloat(maxAmount)) return false;
      if (fromDate && est.date < fromDate) return false;
      if (toDate && est.date > toDate) return false;

      return true;
    });
  }, [estimates, statusFilter, periodFilter, searchQuery, customerFilter, minAmount, maxAmount, fromDate, toDate]);

  // Unique list of customers for select filter
  const uniqueCustomers = useMemo(() => {
    return Array.from(new Set(estimates.map(e => e.customer)));
  }, [estimates]);
    ...
    <section className="view">
      <div className="masthead" style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          ...
          <h1>Estimates</h1>
          <p className="masthead-desc">Manage and track your quotes and sales estimates</p>
        </div>
        <div>
          {canAdd && (
            <button 
          ...
          <div className="stat-grid cols-4" style={{ gap: 16 }}>
            <div>
              <div className="rounded-3 h-100" style={{ border: "1px solid #e2e8f0", borderTop: "3px solid #7baaf7", background: "linear-gradient(135deg, #ffffff 0%, #eef4ff 100%)", padding: "12px 16px" }}>
                <div style={{ fontSize: "0.7rem", letterSpacing: "0.5px", textTransform: "uppercase", fontWeight: 600, color: "var(--muted-2)" }}>Total Estimates</div>
                <div style={{ fontSize: "1.3rem", fontWeight: "bold", color: "var(--ink)", marginTop: 4 }}>INR {metrics.total.toFixed(2)}</div>
                <small style={{ fontSize: "11px", color: "var(--muted)" }}>{estimates.length} estimates</small>
              </div>
            </div>
            ...
            <div>
              <div className="rounded-3 h-100" style={{ border: "1px solid #e2e8f0", borderTop: "3px solid #fbbf24", background: "linear-gradient(135deg, #ffffff 0%, #fffbeb 100%)", padding: "12px 16px" }}>
                <div style={{ fontSize: "0.7rem", letterSpacing: "0.5px", textTransform: "uppercase", fontWeight: 600, color: "var(--muted-2)" }}>Under Review</div>
                <div style={{ fontSize: "1.3rem", fontWeight: "bold", color: "#d97706", marginTop: 4 }}>INR {metrics.underReview.toFixed(2)}</div>
                <small style={{ fontSize: "11px", color: "var(--muted)" }}>{estimates.filter(e => e.status === "Under review").length} awaiting</small>
              </div>
            </div>
            ...
            <div>
              <div className="rounded-3 h-100" style={{ border: "1px solid #e2e8f0", borderTop: "3px solid #4ade80", background: "linear-gradient(135deg, #ffffff 0%, #f0fff4 100%)", padding: "12px 16px" }}>
                <div style={{ fontSize: "0.7rem", letterSpacing: "0.5px", textTransform: "uppercase", fontWeight: 600, color: "var(--muted-2)" }}>Approved</div>
                <div style={{ fontSize: "1.3rem", fontWeight: "bold", color: "var(--income)", marginTop: 4 }}>INR {metrics.approved.toFixed(2)}</div>
                <small style={{ fontSize: "11px", color: "var(--muted)" }}>{estimates.filter(e => e.status === "Approved").length} approved</small>
              </div>
            </div>
            ...
            <div>
              <div className="rounded-3 h-100" style={{ border: "1px solid #e2e8f0", borderTop: "3px solid #a78bfa", background: "linear-gradient(135deg, #ffffff 0%, #f5f0ff 100%)", padding: "12px 16px" }}>
                ...
                <small style={{ fontSize: "11px", color: "var(--muted)" }}>{estimates.filter(e => e.date.startsWith("2026-07")).length} estimates</small>
              </div>
            </div>
          </div>
        </div>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 15, flexWrap: "wrap", marginBottom: 15 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              >
                ...
                <option value="2">Converted to so</option>
              ...
              <span style={{ fontSize: "13px", color: "var(--muted)", whiteSpace: "nowrap" }}>
                {filteredEstimates.length} results
              </span>
            </div>
                ...
                {filteredEstimates.length === 0 ? (
                  <tr className="empty-table-row">
                    <td colSpan={6} style={{ textAlign: "center", padding: "40px 0", color: "var(--muted)" }}>
                      No Estimates present.
                    </td>
                  </tr>
                ) : (
                  filteredEstimates.map((est) => (
                    <tr key={est.id}>
                      <td style={{ fontWeight: 600, color: "var(--ink)" }}>{est.id}</td>
                      <td>{est.date}</td>
                      <td>{est.customer}</td>
                      <td style={{ fontWeight: 600 }}>INR {est.amount.toFixed(2)}</td>
                      <td>
                        <span 
                          style={{
                            display: "inline-flex",
                            ...
                            background: est.status === "Approved" ? "#def7ec" : est.status === "Under review" ? "#fef3c7" : est.status === "Reviewed" ? "#e0e7ff" : "#f3f4f6",
                            color: est.status === "Approved" ? "#03543f" : est.status === "Under review" ? "#d97706" : est.status === "Reviewed" ? "#4f46e5" : "#374151"
                          }}
                        >
                          {est.status}
                        </span>
                      ...
                      <td style={{ textAlign: "center" }}>
                        <button 
                          ...
                          onClick={() => toast.info(`Viewing ${est.id} details`)}
