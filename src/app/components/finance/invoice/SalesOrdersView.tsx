...
export function SalesOrdersView({ onNewSalesOrder, canAdd = true }: { onNewSalesOrder: () => void; canAdd?: boolean }) {
  ...
  const [orders, setOrders] = useState<Array<{ id: string; date: string; customer: string; amount: number; status: string; billingStatus: string }>>([]);

  useEffect(() => {
    (async () => {
      try {
        const docs = await ledgerFinanceService.getDocuments("SALES_ORDER");
  ...
  const metrics = useMemo(() => {
    ...
    orders.forEach((ord) => {
      total += ord.amount;
      if (ord.status === "Open") open += ord.amount;
      if (ord.status === "Executed") executed += ord.amount;
      if (ord.date.startsWith("2026-07")) thisMonth += ord.amount;
    });

    return { total, open, executed, thisMonth };
  }, [orders]);
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter((ord) => {
      ...
      if (searchQuery && !ord.id.toLowerCase().includes(searchQuery.toLowerCase()) && !ord.customer.toLowerCase().includes(searchQuery.toLowerCase())) return false;

      // Period filter
      if (periodFilter === "This Month" && !ord.date.startsWith("2026-07")) return false;
      if (periodFilter === "Last Month" && !ord.date.startsWith("2026-06")) return false;

      // Advanced filters
      if (customerFilter && ord.customer !== customerFilter) return false;
      if (minAmount && ord.amount < parseFloat(minAmount)) return false;
      if (maxAmount && ord.amount > parseFloat(maxAmount)) return false;
      if (fromDate && ord.date < fromDate) return false;
      if (toDate && ord.date > toDate) return false;
      if (billingStatusFilter && ord.billingStatus !== billingStatusFilter) return false;

      return true;
    });
  }, [orders, statusFilter, periodFilter, searchQuery, customerFilter, minAmount, maxAmount, fromDate, toDate, billingStatusFilter]);

  // Unique list of customers for select filter
  const uniqueCustomers = useMemo(() => {
    return Array.from(new Set(orders.map(o => o.customer)));
  }, [orders]);
  ...
  return (
    <section className="view">
      <div className="masthead" style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <p className="masthead-eyebrow">Income / Sales Orders</p>
          <h1>Sales Orders</h1>
          <p className="masthead-desc">Manage and track your customer sales orders</p>
        </div>
        <div>
          {canAdd && (
            <button 
          ...
          <div className="stat-grid cols-4" style={{ gap: 16 }}>
            <div>
              <div className="rounded-3 h-100" style={{ border: "1px solid #e2e8f0", borderTop: "3px solid #7baaf7", background: "linear-gradient(135deg, #ffffff 0%, #eef4ff 100%)", padding: "12px 16px" }}>
                <div style={{ fontSize: "0.7rem", letterSpacing: "0.5px", textTransform: "uppercase", fontWeight: 600, color: "var(--muted-2)" }}>Total Orders</div>
                <div style={{ fontSize: "1.3rem", fontWeight: "bold", color: "var(--ink)", marginTop: 4 }}>INR {metrics.total.toFixed(2)}</div>
                <small style={{ fontSize: "11px", color: "var(--muted)" }}>{orders.length} orders</small>
              </div>
            </div>
            ...
            <div>
              <div className="rounded-3 h-100" style={{ border: "1px solid #e2e8f0", borderTop: "3px solid #fbbf24", background: "linear-gradient(135deg, #ffffff 0%, #fffbeb 100%)", padding: "12px 16px" }}>
                ...
                <small style={{ fontSize: "11px", color: "var(--muted)" }}>{orders.filter(o => o.status === "Open").length} pending</small>
              </div>
            </div>
            ...
            <div>
              <div className="rounded-3 h-100" style={{ border: "1px solid #e2e8f0", borderTop: "3px solid #4ade80", background: "linear-gradient(135deg, #ffffff 0%, #f0fff4 100%)", padding: "12px 16px" }}>
                <div style={{ fontSize: "0.7rem", letterSpacing: "0.5px", textTransform: "uppercase", fontWeight: 600, color: "var(--muted-2)" }}>Executed</div>
                <div style={{ fontSize: "1.3rem", fontWeight: "bold", color: "var(--income)", marginTop: 4 }}>INR {metrics.executed.toFixed(2)}</div>
                <small style={{ fontSize: "11px", color: "var(--muted)" }}>{orders.filter(o => o.status === "Executed").length} completed</small>
              </div>
            </div>
            ...
            <div>
              <div className="rounded-3 h-100" style={{ border: "1px solid #e2e8f0", borderTop: "3px solid #a78bfa", background: "linear-gradient(135deg, #ffffff 0%, #f5f0ff 100%)", padding: "12px 16px" }}>
                ...
                <small style={{ fontSize: "11px", color: "var(--muted)" }}>{orders.filter(o => o.date.startsWith("2026-07")).length} orders</small>
              </div>
            </div>
          </div>
        </div>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 15, flexWrap: "wrap", marginBottom: 15 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              >
                ...
                <option value="2">Partially executed</option>
          ...
          {showAdvanced && (
            <div style={{ border: "1px solid var(--line)", borderRadius: "8px", padding: "16px 20px", background: "#fafbfc", marginBottom: 15, animation: "fadeIn 0.2s" }}>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 15 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  <label style={{ fontSize: "11px", fontWeight: "bold", color: "var(--ink)" }}>Billing Status</label>
                  <select 
                ...
                {filteredOrders.length === 0 ? (
                  <tr className="empty-table-row">
                    <td colSpan={7} style={{ textAlign: "center", padding: "40px 0", color: "var(--muted)" }}>
                      No Sales orders present.
                    </td>
                  </tr>
                ) : (
                  filteredOrders.map((ord) => (
                    <tr key={ord.id}>
                      <td style={{ fontWeight: 600, color: "var(--ink)" }}>{ord.id}</td>
                      <td>{ord.date}</td>
                      <td>{ord.customer}</td>
                      <td style={{ fontWeight: 600 }}>INR {ord.amount.toFixed(2)}</td>
                      <td>
                        <span 
                          style={{
                            display: "inline-flex",
                            ...
                            background: ord.status === "Executed" ? "#def7ec" : ord.status === "Open" ? "#fef3c7" : ord.status === "Partially executed" ? "#e0e7ff" : "#f3f4f6",
                            color: ord.status === "Executed" ? "#03543f" : ord.status === "Open" ? "#d97706" : ord.status === "Partially executed" ? "#4f46e5" : "#374151"
                          }}
                        >
                          {ord.status}
                        </span>
                      ...
                      <td>
                        <span 
                          style={{
                            ...
                            fontWeight: "bold",
                            textTransform: "uppercase",
                            background: ord.billingStatus === "Billed" ? "#def7ec" : ord.billingStatus === "Unbilled" ? "#fde8e8" : "#e0e7ff",
                            color: ord.billingStatus === "Billed" ? "#03543f" : ord.billingStatus === "Unbilled" ? "#9b1c1c" : "#4f46e5"
                          }}
                        >
                          {ord.billingStatus}
                        </span>
                      ...
                      <td style={{ textAlign: "center" }}>
                        <button 
                          ...
                          onClick={() => toast.info(`Viewing ${ord.id} details`)}
