    ...
    <>
      ...
      <div className="flex items-center gap-1 sm:gap-1.5">
        ...
        <button
          ...
          className="p-1.5 sm:p-2 rounded-lg bg-sky-50 border border-sky-100/80 text-sky-600 hover:bg-sky-100 active:scale-95 transition-all flex items-center justify-center cursor-pointer lg:hidden"
        >
          <Search className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-sky-600" />
        </button>
        ...
        {onToggleSidebar && (
          <button
            ...
            className="p-1.5 sm:p-2 rounded-lg bg-slate-100/90 border border-slate-200 text-slate-700 hover:bg-slate-200 active:scale-95 transition-all flex items-center justify-center cursor-pointer lg:hidden"
          >
            <MenuListIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-700" />
          </button>
        )}
      ...
      {activeModal === "search" && (
        <div className="fixed inset-x-2.5 top-6 bottom-3 sm:inset-x-3 sm:top-10 sm:bottom-6 z-50 flex flex-col bg-white rounded-xl sm:rounded-2xl shadow-2xl border border-slate-200 overflow-hidden animate-in fade-in zoom-in-95 duration-200 lg:inset-auto lg:top-14 lg:left-1/2 lg:-translate-x-1/2 lg:w-[540px] lg:max-h-[80vh]">
          {/* Search Header */}
          <div className="p-2 sm:p-2.5 bg-gradient-to-r from-sky-600 via-sky-500 to-indigo-600 text-white flex items-center gap-2">
            <div className="flex-1 flex items-center gap-2 bg-white/15 border border-white/25 rounded-lg px-2.5 py-1.5 text-white">
          ...
          <div className="flex-1 overflow-y-auto p-2 sm:p-2.5 space-y-1.5">
            {filteredItems.length === 0 ? (
              <div className="text-center py-8 px-4">
                ...
                <p className="text-[10.5px] text-slate-500 mt-0.5">Try searching for events, sports, marketplace, notices, or bookings.</p>
              </div>
            ) : (
              <>
                <div className="flex items-center justify-between px-1 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                  ...
                  <span className="text-[9px] font-normal lowercase">{filteredItems.length} destinations</span>
                </div>
                <div className="space-y-1">
                  {filteredItems.map((item) => (
                    <button
                      ...
                      className="w-full p-1.5 sm:p-2 rounded-lg sm:rounded-xl border border-slate-100 bg-white hover:bg-slate-50/80 hover:border-sky-200 transition-all flex items-center gap-2 sm:gap-2.5 text-left group shadow-2xs cursor-pointer active:scale-[0.99]"
                    >
                      <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-lg ${item.bg} flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-[8.5px] font-semibold px-1 py-0.2 bg-slate-100 text-slate-500 rounded leading-none">
                            {item.category}
                          </span>
                        </div>
                      ...
                      <ChevronRight className="w-3 h-3 text-slate-300 group-hover:text-sky-500 group-hover:translate-x-0.5 transition-all shrink-0" />
                    </button>
                  ))}
                </div>
              </>
      ...
      {activeModal === "directory" && (
        <div className="fixed inset-x-2.5 top-6 bottom-3 sm:inset-x-4 sm:top-10 sm:bottom-3 z-50 flex flex-col bg-slate-50 rounded-xl sm:rounded-2xl shadow-2xl border border-slate-200 overflow-hidden animate-in fade-in zoom-in-95 duration-200 lg:inset-auto lg:top-16 lg:right-12 lg:w-[680px] lg:max-h-[85vh]">
          {/* Header */}
          <div className="px-2.5 py-1.5 sm:px-3 sm:py-2 bg-gradient-to-r from-emerald-600 via-teal-600 to-emerald-700 text-white flex items-center justify-between shrink-0 shadow-xs">
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded-lg bg-white/15 border border-white/20 flex items-center justify-center shadow-2xs shrink-0">
                <Users className="w-3.5 h-3.5 text-emerald-100" />
              </div>
              <div>
                <h3 className="font-bold text-xs text-white leading-tight">Community Directory</h3>
                <p className="text-[9px] text-white/80 leading-snug">Management committee, leaders & emergency contacts</p>
              </div>
            </div>
            <button
              onClick={() => setActiveModal(null)}
              className="w-5.5 h-5.5 rounded-lg hover:bg-white/20 text-white/80 hover:text-white transition-colors flex items-center justify-center cursor-pointer"
            >
