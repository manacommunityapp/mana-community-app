...
// Placeholder data used when the backend KYC-applications endpoint is not yet available.
// TODO: Replace with a real GET /api/admin/kyc-applications call when the endpoint is implemented.
const PLACEHOLDER_APPLICATIONS: UserApplication[] = [
  {
    id: "1",
...
export function AdminDashboard() {
  ...
  useEffect(() => {
    // TODO: Replace with real GET /api/admin/kyc-applications when endpoint is available.
    // For now, simulate a network request then fall back to placeholder data.
    async function fetchApplications() {
      try {
        // Attempt to hit the real endpoint; fall back silently if 404
        const data = await apiClient.get<UserApplication[]>("/admin/kyc-applications");
        setApplications(data);
      } catch {
        // Endpoint not yet implemented — use placeholder data
        setApplications(PLACEHOLDER_APPLICATIONS);
        setApiError("Live data unavailable — showing sample applications. Implement GET /api/admin/kyc-applications to enable real data.");
      } finally {
        setLoading(false);
      }
    }
  ...
  const filteredApplications = applications.filter((app) => {
    ...
    const matchesSearch = !q || app.fullName.toLowerCase().includes(q) || app.email.toLowerCase().includes(q);
  ...
  const handleApprove = async (id: string) => {
    try {
      // TODO: call PATCH /api/admin/kyc-applications/{id}/approve when endpoint is available.
      await apiClient.patch(`/admin/kyc-applications/${id}/approve`);
    } catch {
      // Endpoint not implemented — optimistic local update
    }
  ...
  const handleReject = async (id: string) => {
    try {
      // TODO: call PATCH /api/admin/kyc-applications/{id}/reject when endpoint is available.
      await apiClient.patch(`/admin/kyc-applications/${id}/reject`);
    } catch {
      // Endpoint not implemented — optimistic local update
    }
    setApplications((prev) =>
