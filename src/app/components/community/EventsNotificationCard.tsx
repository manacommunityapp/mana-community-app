  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200/90 shadow-xs overflow-hidden transition-all duration-300 hover:shadow-md">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <span className="text-xs text-slate-400 font-medium">Upcoming community festivals</span>
          </div>
        </div>
        <div className="flex items-center gap-1.5">
          {currentEventsCount > 0 && (
            <span className="px-1.5 py-0.5 rounded-full text-xs font-black bg-indigo-50 text-indigo-600 border border-indigo-100 leading-none">
              {currentEventsCount}
            </span>
          )}
          {expanded ? (
      ...
      <div className="px-3 py-2">
        {loading ? (
          <div className="flex items-center justify-center py-3 gap-1.5 text-xs text-slate-400">
            <Loader2 className="w-3.5 h-3.5 animate-spin text-indigo-500" />
            Loading…
          </div>
        ) : events.length === 0 ? (
          <div className="text-center py-3">
            <p className="text-xs font-semibold text-slate-500">No upcoming events</p>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-bold text-slate-800 truncate group-hover:text-indigo-600 transition-colors leading-tight">
                      {ev.title}
                    </p>
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      {ev.venue && (
                        <span className="flex items-center gap-0.5 text-xs text-slate-400 truncate max-w-[80px]">
      ...
      {!loading && events.length > 0 && (
        <button
          type="button"
          onClick={() => navigate("/events")}
          className="w-full flex items-center justify-center gap-1 py-2 text-xs font-bold text-indigo-500 hover:text-indigo-700 hover:bg-indigo-50 border-t border-slate-100 transition-colors cursor-pointer"
        >
