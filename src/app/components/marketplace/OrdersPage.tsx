...
import {
  ...
  QrCode, KeyRound, Check, X, Shield, MapPin, CreditCard, Banknote
} from "lucide-react";
import { orderService, type OrderResponse } from "../../../services/marketplace/listingService";
...
export function OrdersPage() {
  ...
  const [verifyingOrder, setVerifyingOrder] = useState<OrderResponse | null>(null);
  ...
  useEffect(() => {
    ...
    if (USE_MOCK_DATA) {
      const filtered = tab === "buying"
        ? MOCK_ORDERS.filter((o) => o.buyer.fullName === "Demo User" || o.buyer.fullName === "Demo Resident")
        : MOCK_ORDERS.filter((o) => o.seller.fullName === "Demo User" || o.seller.fullName === "Demo Resident" || o.seller.id === 100);
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      if (USE_MOCK_DATA) {
        if (verifyingOrder.pickupOtp && inputOtp !== verifyingOrder.pickupOtp) {
          showError("Invalid Pickup OTP entered. Please check with resident.");
          setVerifying(false);
          return;
        }
        setOrders((prev) =>
          prev.map((o) => (o.id === verifyingOrder.id ? { ...o, status: "COMPLETED", paymentStatus: "PAID" } : o))
        );
      } else {
        await orderService.verifyOtp(verifyingOrder.id, inputOtp);
        setOrders((prev) =>
          prev.map((o) => (o.id === verifyingOrder.id ? { ...o, status: "COMPLETED", paymentStatus: "PAID" } : o))
        );
      }
      showSuccess("OTP Verified! Package successfully handed over.");
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {orders.map((order) => {
            ...
            const hasGateOtp = order.pickupOtp && ["GATE_SECURITY", "CLUBHOUSE"].includes(order.deliveryMethod || "");
            ...
            return (
              <div key={order.id} className="bg-white dark:bg-[#1E1E36] rounded-3xl border border-slate-200 dark:border-slate-800 p-5 shadow-xs space-y-4">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 dark:border-slate-800 pb-3">
                  ...
                  <div className="flex items-center gap-2">
                    {order.paymentStatus === "PAID" ? (
                      <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 dark:bg-emerald-950/40 px-2.5 py-0.5 rounded-full border border-emerald-200 dark:border-emerald-800">
                        Paid ({order.paymentMode || "UPI"})
                      </span>
                    ) : (
                      <span className="text-[10px] font-bold text-amber-600 bg-amber-50 dark:bg-amber-950/40 px-2.5 py-0.5 rounded-full border border-amber-200 dark:border-amber-800">
                        Payment Pending
                      </span>
                    )}
                ...
                {/* Details */}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-slate-50 dark:bg-[#262644] p-3.5 rounded-2xl gap-3">
                  <div>
                    <h4 className="text-xs font-bold text-slate-800 dark:text-slate-200">{firstItem?.listingTitle || "Marketplace Product"}</h4>
                    <p className="text-[10px] text-slate-400">Qty: {firstItem?.quantity || 1} • {order.deliveryAddress}</p>
                  </div>
                ...
                {/* Gate Security / Clubhouse Handover Pass (OTP & QR) */}
                {hasGateOtp && order.status !== "COMPLETED" && order.status !== "CANCELLED" && (
                  <div className="bg-indigo-50/50 dark:bg-indigo-950/30 border border-indigo-200 dark:border-indigo-800 p-3.5 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-indigo-600 text-white flex items-center justify-center shrink-0">
                        <KeyRound className="w-5 h-5" />
                      </div>
                      <div>
                        <div className="text-xs font-black text-indigo-900 dark:text-indigo-200 flex items-center gap-1">
                          Gate Security Handover Pass
                        </div>
                        <p className="text-[10px] text-slate-500 dark:text-slate-400">
                          {tab === "buying"
                            ? "Show this 4-digit PIN to the security guard/clubhouse desk when collecting your parcel."
                            : "Enter this OTP when handing over package at security gate."}
                        </p>
                      </div>
                    </div>

                    <div className="flex items-center gap-2">
                      <div className="px-3.5 py-1.5 bg-white dark:bg-[#1E1E36] rounded-xl border border-indigo-200 dark:border-indigo-800 font-mono text-sm font-black tracking-widest text-indigo-600 dark:text-indigo-400">
                        {order.pickupOtp}
                      </div>
                      {tab === "selling" && (
                        <button
          ...
          <div className="bg-white dark:bg-[#1E1E36] rounded-3xl p-6 w-full max-w-sm border border-slate-200 dark:border-slate-800 shadow-2xl space-y-4 text-center">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
              <h3 className="text-sm font-black text-slate-900 dark:text-white flex items-center gap-1.5">
                <ShieldCheck className="w-4 h-4 text-emerald-500" /> Verify Gate Pickup OTP
              </h3>
              <button onClick={() => setVerifyingOrder(null)} className="p-1 text-slate-400 hover:text-slate-600">
                <X className="w-5 h-5" />
              </button>
            </div>

            <form onSubmit={handleVerifyOtp} className="space-y-3">
              <p className="text-xs text-slate-400">Ask the collecting resident or guard for their 4-digit pickup code.</p>
              <input
                type="text"
                maxLength={4}
                required
                ...
                className="w-full text-center tracking-widest font-mono text-lg font-black px-4 py-2.5 bg-slate-50 dark:bg-[#262644] border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:border-indigo-600 text-slate-900 dark:text-white"
              />
              ...
              <div className="flex gap-2 pt-2">
                <button
                  ...
                  className="flex-1 py-2 text-xs font-bold text-slate-400 hover:bg-slate-100 dark:hover:bg-[#262644] rounded-xl"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 py-2 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white text-xs font-bold rounded-xl"
                >
